...
| 20 | ✅ **FIXED**, and its rationale partly **superseded by `F-028`** (2026-08-01). A review can reach a state with no exit; the only escape is defect 19. **[R]** Fixed: `review_pending → active`, and `work resume` covers it — the revocation `handed_off → active` already allowed, one stage later. This entry went on to say that when the branch moves after the handoff no verdict is recordable, *correctly*. That reasoning no longer holds and the behaviour is gone: a verdict that found problems is a true statement about the candidate it was reached against, and it stays true when the branch moves, so refusing to file it destroyed the reviewer's work to protect an invariant only approvals carry. Found by making the mistake four times on one card, where three review rounds survive only as prose inside a `handoff revoke --reason`. A non-approval is now recordable after the branch moves; an approval is still refused. `F-030` (landed via `INT-030`/`ACC-000030` at `e2866a7`) closed the rest across four review rounds. Round 1 rejected narrowing the relaxation to the candidate SHA alone: revocation is a delivery-side control, and refusing a non-approval against a revoked handoff would let an implementer suppress an adverse verdict by revoking before the reviewer files — the exact failure this defect exists to prevent. So revocation stays unchecked for a non-approval, deliberately, and a test now asserts it rather than leaving it to be rediscovered. Round 2 found the fix had only reached `blocked` — `active` had no edge to `changes_requested`, so revoking still picked which verdict a reviewer was allowed to reach; `active → changes_requested` is now permitted for the same reason `review_pending` gained `Blocked` and `Active` above. What a non-approval still answers for is the card itself: a card revised out from under its handoff refuses with `CH-POLICY-STALE-HANDOFF` naming the digest, not the transition table's message. See D-093. Round 3 found one more gap the relaxation leans on rather than closes: `review record` has never checked that `review begin` happened, so a revoked handoff with no review round at all still accepts a verdict. `F-030` widens that from `blocked` to both verdicts and states so outright in a test, rather than pretending otherwise; closing it is tracked separately as `F-037`. |
