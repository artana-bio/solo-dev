...
| D-083, D-086 – D-091 | Record-hygiene decisions, split out of `F-027` | In progress | The work is filed as `artana-bio/solo-dev#13`, with the prior implementation preserved at tag `f027-full-with-hygiene`. The first safe slice is bounded to the Git-object encoding boundary: retain raw Git output and refuse staged control entries that are invalid UTF-8 or contain NUL bytes, covering the known UTF-16 bypass. Secret-shape recognition, generated-output redaction, overrides, and the other issue findings remain out of scope for this slice. |
