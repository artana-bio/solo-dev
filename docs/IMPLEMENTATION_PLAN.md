...
| D-083, D-086 – D-091 | Record-hygiene decisions, split out of `F-027` | Accepted — minimum policy; see D-094 | The bounded implementation remains the quarantined post-filter byte and JSON-tree inspection slice described here. D-094 records the accepted policy boundary: clean intended entries under the built-in hygiene policy are allowed; a claimed override or acknowledgment is refused because no such capability exists; and a mixed sensitive-plus-clean transaction is refused as a whole, with the sensitive input removed and the transaction retried. There is no partial commit or refusal event. This status does not claim a runtime policy digest, override/acknowledgment capability, new credential classes, or a recovery redesign. |
...
| D-094 | Freeze the #43 minimum record-hygiene policy at candidate `3f405ff932e058be487607ff6c2b1322cac546f5` | Accepted 2026-08-02 by Alvaro Alvarez | A clean transaction under the built-in hygiene policy is `ALLOW`. A claimed override or acknowledgment is `REFUSE`: no override or acknowledgment capability exists. A mixed transaction containing sensitive and clean intended entries is `REFUSE` as a whole; remove or isolate the sensitive input and retry. No partial commit and no refusal event is created. This is a documentation-only successor decision tied to #43 and the exact base; it does not assert an unimplemented runtime policy digest, override mechanism, additional credential classes, output redaction, path handling, or recovery redesign. |
