...
| Evidence | Review repair for `RV-000092`: cleaned mutation failures now preserve the original classified error in a durable `failed_clean` `mutation.create` journal record after verified disposable-worktree restoration. Committed-gate recovery now reads and binds the deleted historical permit, requires its holder and key to match the reservation, binds each event ID to its committed path, and rejects unsupported or structurally invalid receipts even when their digest is recomputed. Focused checks passed: mutation 7/7 and gate runner 33/33. `cargo fmt --check`, strict all-target/all-feature Clippy, and the normalized complete `cargo test` suite passed. Four declared negative mutations each made its owning regression fail before exact restoration. Exact-SHA feature receipts and the replacement handoff remain authoritative in control state rather than being copied into this mutable tracker. |
