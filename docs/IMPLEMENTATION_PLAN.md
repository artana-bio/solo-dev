...
| D-083, D-086 – D-091 | Record-hygiene decisions, split out of `F-027` | In progress | The work is filed as `artana-bio/solo-dev#13`, with the prior implementation preserved at tag `f027-full-with-hygiene`. The first safe slice is bounded to the Git-object encoding boundary: stage only the existing intended control paths through a quarantined index/object store, inspect the exact post-filter bytes, and refuse entries that are invalid UTF-8 or contain NUL bytes before promoting those objects into the durable repository. Promotion acquires `.git/index.lock` with create-new semantics, fsyncs the validated index into that owned lock, and installs it atomically; refusal or lock/promotion failure leaves the durable index and object database untouched, covering the known UTF-16, clean-filter, and index-lock bypasses. Secret-shape recognition, generated-output redaction, overrides, and the other issue findings remain out of scope for this slice. |
