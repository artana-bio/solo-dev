...
| D-083, D-086 – D-091 | Record-hygiene decisions, split out of `F-027` | In progress — current behavior disclosed; target D-094 | The current implementation is limited to the quarantined post-filter byte and JSON-tree inspection slice. Clean intended entries under the built-in hygiene policy are allowed, and no override or acknowledgment capability exists, so a claimed one is refused. A command-generated sensitive record can still leave its cycle/event plus a `failed_partial` journal record, and an identical retry requires recovery; the sensitive content does not reach a Git commit. D-094's mixed-entry no-residue/direct-retry behavior is the approved target policy, not implemented current behavior. |
...
| D-094 | Freeze the #43 minimum record-hygiene policy at candidate `3f405ff932e058be487607ff6c2b1322cac546f5` | Approved target policy — not implemented | The approved TARGET policy is: a clean transaction under the built-in hygiene policy is `ALLOW`; a claimed override or acknowledgment is `REFUSE` because no such capability exists; and a mixed transaction containing sensitive and clean intended entries is `REFUSE` as a whole, with the sensitive input removed or isolated before retry. The target includes no partial commit and no refusal event. This documentation-only successor decision is tied to #43 and the exact base; it does not assert that the mixed-entry no-residue/direct-retry behavior, a runtime policy digest, an override mechanism, additional credential classes, output redaction, path handling, or recovery redesign exists today. |
