...
fn tiered_review_lesson_definition(
    workspace: &Workspace,
    enforcement: &str,
    source_id: &str,
) -> String {
    let path = workspace.root.join(format!("{enforcement}-lesson.yaml"));
    fs::write(
        &path,
        format!(
            "title: {enforcement} review guidance\nrule: Record the review disposition\nrationale: Guidance must remain visible\nselectors:\n  paths: [src/**]\n  contracts: []\n  change_kinds: []\n  minimum_risk: null\nenforcement: {enforcement}\nobligations:\n  feature_gates: []\n  integration_gates: []\n  review_checks: [tier-check]\nprovenance:\n  source_kind: review\n  source_id: {source_id}\n  evidence: Tier semantics regression\n"
        ),
    )
    .unwrap();
    path.display().to_string()
...
fn prepare_lesson_review(
    ...
    workspace.cycle(&[
        ...
        "C-001",
        "--objective",
        "Exercise lesson review semantics",
    ]);
    workspace.cycle(&["activate", "--cycle-id", "C-001"]);
    propose_and_activate(
        workspace,
        tiered_review_lesson_definition(workspace, enforcement, "RV-TIER"),
    );
    ...
    workspace.root.join(format!("{enforcement}-verdict.yaml"))
}
...
#[test]
fn retiring_a_lesson_after_activation_does_not_rewrite_the_frozen_manifest() {
    let workspace = Workspace::initialized();
    install_lesson_authorizer(&workspace, "owner");
    workspace.cycle(&[
        "create",
    ...
    workspace.activate_card_with_gates("F-001", &["src/**"], &["gate.unit"]);
    workspace.work(&["start", "--card-id", "F-001"]);
    let frozen_digest = packet_digest(&workspace);
    ...
    for dry_run in [true, false] {
        ...
        assert!(
            output.status.success(),
            "retiring current policy must not rewrite activation evidence: {}{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
        if !dry_run {
...
fn a_later_lesson_activation_does_not_invalidate_a_frozen_handoff_manifest() {
    ...
    workspace.cycle(&["activate", "--cycle-id", "C-001"]);
    workspace.activate_card("F-001", &["src/**"]);
    let binding_path = workspace
        .control
        .join("cards/F-001/lesson-manifests/1.json");
    let binding_before = fs::read(&binding_path).expect("activation-time lesson binding");
    workspace.work(&["start", "--card-id", "F-001"]);
    ...
    assert_eq!(
        ...
        binding_before,
        "later lesson policy must not rewrite activation evidence"
    );
...
fn preferred_review_checks_require_a_visible_disposition() {
    ...
    assert_eq!(
        envelope["error"]["code"],
        "CH-POLICY-LESSON-EVIDENCE-MISSING"
    );
    assert_eq!(workspace.control_head(), before);
}

#[test]
fn informational_review_checks_remain_non_blocking_context() {
    let workspace = Workspace::initialized();
    let verdict = prepare_lesson_review(&workspace, "informational", "reviewer");
    fs::write(
        &verdict,
        "reviewer_actor_id: reviewer\ndecision: changes_requested\nfindings:\n  - severity: medium\n    location: src/a.rs\n    detail: needs work\n    disposition: open\ngate_adequacy:\n  gates_observe_acceptance: true\n  unobserved_behaviors: []\n  basis: inspected candidate\n  mutation_evidence:\n    status: exempt\n    reason: non-approval fixture\nresidual_risks: []\nreview_conduct: separate_process\n",
    )
    ...
    assert!(
        output.status.success(),
        "informational context must not become a gate: {}{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
#[test]
fn unauthorized_not_applicable_refuses_before_control_mutation() {
    let workspace = Workspace::initialized();
    let verdict = prepare_lesson_review(&workspace, "required", "outsider");
    fs::write(
        &verdict,
        "reviewer_actor_id: outsider\ndecision: changes_requested\nfindings:\n  - severity: medium\n    location: src/a.rs\n    detail: needs work\n    disposition: open\nlesson_checks:\n  - lesson_id: LS-000001\n    check_id: tier-check\n    status: not_applicable\n    evidence: claimed exception\ngate_adequacy:\n  gates_observe_acceptance: true\n  unobserved_behaviors: []\n  basis: inspected candidate\n  mutation_evidence:\n    status: exempt\n    reason: non-approval fixture\nresidual_risks: []\nreview_conduct: separate_process\n",
    )
    .unwrap();
    let before = workspace.control_head();
    let output = workspace.review_raw(&[
    ...
    assert!(!output.status.success());
    let envelope: Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(envelope["error"]["code"], "CH-POLICY-LESSON-DISPOSITION");
    assert_eq!(workspace.control_head(), before);
}
