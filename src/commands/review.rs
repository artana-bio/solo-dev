...
use crate::{
    ...
    domain::{
        ...
        ids::{CardId, ReviewId},
        lesson::{LessonCheck, LessonCheckStatus, LessonEnforcement, LessonManifest},
        mutation::{
...
fn validate_current_lesson_manifest(
    ...
    validate_frozen_card_manifest(control, card, manifest)
}

/// Requires an explicit disposition for every required or preferred review
/// check. Informational lessons remain non-blocking context.
fn require_lesson_checks(
    ...
    require_review_dispositions(manifest, verdict, authorization)
}

fn require_review_dispositions(
    ...
    for lesson in manifest
        .lessons
        .iter()
        .filter(|lesson| lesson.enforcement != LessonEnforcement::Informational)
    {
        for check_id in &lesson.obligations.review_checks {
            let Some(check) = verdict
                ...
                return Err(HarnessError::Control {
                    reason: format!(
                        "{} lesson `{}` review check `{check_id}` has no disposition",
                        lesson.enforcement.name(),
                        lesson.lesson_id
                    ),
                    code: ErrorCode::PolicyLessonEvidenceMissing,
                });
            };
            if lesson.enforcement == LessonEnforcement::Required
                && check.status == LessonCheckStatus::NotSatisfied
            {
                return Err(HarnessError::Control {
                    reason: format!(
            ...
            if check.status == LessonCheckStatus::NotApplicable {
                let policy = authorization.ok_or_else(|| HarnessError::ControlWithRecovery {
                    reason: format!(
                        "{} lesson `{}` review check `{check_id}` is marked not_applicable, but final authorization is not configured for this project",
                        lesson.enforcement.name(), lesson.lesson_id
                    ),
                ...
                if !policy.authorizes(&verdict.reviewer_actor_id) {
                    return Err(HarnessError::ControlWithRecovery {
                        reason: format!(
                            "reviewer {} is not configured to authorize not_applicable for {} lesson `{}` review check `{check_id}`",
                            verdict.reviewer_actor_id,
                            lesson.enforcement.name(),
                            lesson.lesson_id
                        ),
