...
use crate::{
    ...
    domain::{
        ...
        ids::{CardId, LessonId},
        lesson::{
            CARD_LESSON_BINDING_SCHEMA, CardLessonBindingRecord, LESSON_DIR, LESSON_SCHEMA,
            LessonDraft, LessonEnforcement, LessonManifest, LessonObligations, LessonProvenance,
            LessonRecord, LessonSelectors, LessonStatus,
        },
    },
...
/// Freezes the applicable lesson set in the transaction that activates or
/// revises the immutable card record.
pub(crate) fn freeze_card_manifest(
    control: &ControlRepository,
    card: &crate::domain::card::CardRecord,
    card_digest: &crate::domain::digest::Digest,
) -> Result<CardLessonBindingRecord, HarnessError> {
    let path = CardLessonBindingRecord::relative_path(&card.card_id, card.revision);
    if control.path(&path).exists() {
        return Err(HarnessError::Control {
            reason: format!(
                "card {} revision {} already has a frozen lesson binding",
    ...
    let binding = CardLessonBindingRecord {
        schema: CARD_LESSON_BINDING_SCHEMA.to_owned(),
...
/// Loads and validates the manifest frozen for this exact card revision.
pub(crate) fn frozen_card_manifest(
    control: &ControlRepository,
    card: &crate::domain::card::CardRecord,
) -> Result<LessonManifest, HarnessError> {
    let path = CardLessonBindingRecord::relative_path(&card.card_id, card.revision);
    let raw = control.read(&path).map_err(|_| HarnessError::Control {
        reason: format!(
            "card {} revision {} is missing its activation-time lesson binding",
            card.card_id, card.revision
        ),
        code: ErrorCode::PolicyLessonManifestStale,
    })?;
    let binding: CardLessonBindingRecord =
        serde_json::from_str(&raw).map_err(|source| HarnessError::Control {
            reason: format!(
                "card {} revision {} lesson binding is malformed: {source}",
                card.card_id, card.revision
            ),
            code: ErrorCode::PolicyLessonManifestStale,
        })?;
    ...
    if binding.card_id != card.card_id
        || binding.card_revision != card.revision
        || binding.card_digest != card.digest()?
    {
        return Err(HarnessError::Control {
            reason: format!(
                "card {} revision {} lesson binding names different card evidence",
                card.card_id, card.revision
            ),
            code: ErrorCode::PolicyLessonManifestStale,
        });
    }
    validate_manifest_registry(control, &binding.manifest)?;
    crate::policy::lessons::validate_manifest_for_card(&binding.manifest, card)?;
    Ok(binding.manifest)
}

/// Requires a downstream packet to equal the activation-time binding.
pub(crate) fn validate_frozen_card_manifest(
    control: &ControlRepository,
    card: &crate::domain::card::CardRecord,
    manifest: &LessonManifest,
) -> Result<(), HarnessError> {
    if frozen_card_manifest(control, card)? != *manifest {
        return Err(HarnessError::Control {
            reason: format!(
                "card {} revision {} downstream lesson manifest differs from its activation-time binding",
                card.card_id, card.revision
            ),
            code: ErrorCode::PolicyLessonManifestStale,
        });
...
/// Resolves the exact immutable lesson revisions embedded in one manifest.
pub(crate) fn lesson_records_for_manifest(
    control: &ControlRepository,
    manifest: &LessonManifest,
) -> Result<Vec<LessonRecord>, HarnessError> {
    validate_manifest_registry(control, manifest)?;
    let mut records = Vec::with_capacity(manifest.lessons.len());
    for selected in &manifest.lessons {
        let record = lesson_history(control, &selected.lesson_id)?
            .into_iter()
            .find(|record| record.revision == selected.revision)
            .ok_or_else(|| HarnessError::Control {
                reason: format!(
...
fn run_match(args: &MatchArgs) -> Result<CommandOutcome, HarnessError> {
    ...
    let manifest = frozen_card_manifest(&control, &card)?;
